...
const MetricDataViewer = ({ metric }) => {
  ...
  const loadSavedThresholds = (metricName) => {
    try {
      const savedThresholds = localStorage.getItem(`thresholds_${metricName}`);
      if (savedThresholds) {
  ...
  const saveThresholds = (metricName, warning, critical) => {
    try {
      localStorage.setItem(`thresholds_${metricName}`, JSON.stringify({
        warning: warning,
        critical: critical
      }));
    } catch (error) {
  ...
  const loadSavedAlertStatus = (metricName) => {
    try {
      const savedAlertStatus = localStorage.getItem(`alertStatus_${metricName}`);
      if (savedAlertStatus) {
  ...
  const saveAlertStatus = (metricKey) => {
    try {
      ...
      const metricName = metricKey.split('_')[0]; // Extract metric name from key
      localStorage.setItem(`alertStatus_${metricName}`, JSON.stringify(newAlertSent));
    } catch (error) {
  ...
  const fetchMetricData = async () => {
    ...
    try {
      ...
      // Get current value
      const currentResponse = await fetch(`/api/v1/query?query=${encodeURIComponent(metric.name)}`);
      const currentData = await currentResponse.json();
      
      // Get yesterday's value (24 hours ago)
      const yesterdayTimestamp = Math.floor(Date.now() / 1000) - 86400; // 24 hours ago
      const yesterdayResponse = await fetch(`/api/v1/query?query=${encodeURIComponent(metric.name)}&time=${yesterdayTimestamp}`);
